...
func newScore(renderer *sdl.Renderer) (*score, error) {
	...
	s._xPos = windowWidth - 20
	s._yPos = 10
	s._score = 0

	if s._font, err = ttf.OpenFont("fonts/freesansbold.ttf", scoreFontSize); err != nil {
		return s, err
	}
...
func (s *score) increment(points int) error {
	s._score += points
	err := s.update()
	return err
...
func (s *score) update() error {
	...
	if scoreSurf, err = s._font.RenderUTF8Blended("Score: "+strconv.Itoa(s._score), s._fontColor); err != nil {
		return err
	}
...
func (s *score) draw() {
	s._renderer.Copy(s._image, nil, &s._rect)
}
